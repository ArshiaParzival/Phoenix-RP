...
local _source = source
	...
	if Stores[robb] then
		...
		if rob == false then
			...
			TriggerEvent('Parzival:robjavaheri', _source, true)
			CancelEvent()
...
RegisterServerEvent('Parzival:checkvantime')
AddEventHandler('Parzival:checkvantime', function(robb)
if Stores[robb] then
		local store = Stores[robb]
	if (os.time() - store.lastrobbed) > Config.SecBetwNextRob and store.lastrobbed == 0 then
            TriggerEvent('Parzival:robjavaheri', _source, false)
			return
		end
		end
end)
